...
def load_data():
    ...
    cols_to_remove = ['Unnamed: 0', 'Weekday Name']
...
try:
    ...
    if model_choice == "Prophet":
        ...
        if len(df_prophet) > 2:
            ...
            st.error("Not enough data to run Prophet.")

    elif model_choice == "ARIMA":
        st.subheader(f"Analyzing Sales with ARIMA: {selected_drug}")
      
        df_arima = df.set_index('datum')[selected_drug].dropna().sort_index()
        ...
        if len(df_arima) > 5:
            ...
            st.error("Not enough data points to run ARIMA.")
